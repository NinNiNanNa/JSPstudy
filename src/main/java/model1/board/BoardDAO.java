...
public class BoardDAO extends JDBConnect {
	...
	public BoardDAO(ServletContext application) {
		// 부모 생성자에서는 appliction을 통해 web.xml에 직접 접근하여 컨텍스트 초기화 파라미터를 얻어온다.
		super(application);
	}
	...
	*/
	public List<BoardDTO> selectList(Map<String, Object> map){
		...
		/*
		List계열의 컬렉션을 생성한다.
		이때 타입 매개변수는 board테이블을 대상으로하므로 BoardDTO로 설정한다.
		*/
		List<BoardDTO> bbs = new Vector<BoardDTO>();
		
		/*
		레코드 인출을 위한 select 쿼리문 작성.
		최근 게시물이 상단에 출력되야하므로 일련번호의 내림차순으로 정렬한다.
		*/
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map){
		...
		/*
		검색조건에 일치하는 게시물을 얻어온 후
		각 페이지에 출력할 구간까지 설정한 서브 쿼리문 작성
		*/
		String query = "SELECT * FROM ( "
					 + " 	SELECT Tb.*, ROWNUM rNum FROM ( "
					 + " 		SELECT * FROM board ";
		// 검색어가 잇는 경우에만 where절을 추가한다.
		if (map.get("searchWord") != null) {
	...
	public BoardDTO selectView(String num) {
		...
		String query = "SELECT B.*, M.name FROM board B INNER JOIN member M "
					 + " ON B.id=M.id "
					 + " WHERE num=?";
