...
: 외부의 문서를 현재문서에 포함시킨다.
 ...
 -->
	...
	웹프로그리맹은 웹서버에 배포하는것이 목적이므로 서버의 환경이 달라지면 경로까지 수정해야하는 경우 이를 최소화할 수 있도록 개발하는것이 좋다.
	특히 JSTL에서 URL을 지정할때 컨텍스트루트 경로는 명시하지 않아도 되므로 일반적인 방식보다 작성에 유리한 점이 있다.
	 -->
		<h4>이미지 삽입하기</h4>
		<h5>상대경로 지정</h5>
		...
		<h5>컨텍스트 루트명을 가져온 후 절대경로로 삽입하기(내장객체 사용)</h5>
