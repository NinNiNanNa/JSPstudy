...
: 변수를 선언할때 사용.
  ...
  value : 속성에 저장될 값.
  scope : 4가지 영역명을 지정. 미지정시 가장 좁은 page영역에 저장된
  target : set 태그를 통해 생성된 자바빈즈의 이름을 저장함.
  property : target으로 지정한 자바빈즈의 멤버변수를 지정한다.
 -->
	<!-- 변수 선언 : 여기서는 가장 좁은 page영역에 저장된다. -->
	<!-- 
	value에는 일반값, EL, 표혐식을 모두 사용할 수 있다.
	...
	<!-- 
	JSTL은 JSP코드이므로 value속성에 기술할때 객체를 생성하기 위한 더블쿼테이션이 겹쳐지는 경우가 발생하기 된다.
	...
	<!-- 
	빈 객체를 먼저 생성한 후 target, property를 통해 멤버변수의 값을 설정한다.
